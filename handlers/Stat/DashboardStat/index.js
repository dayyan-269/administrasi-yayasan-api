...
const getDashboardStats = async (request, h) => {
  const countAnakAsuhan = await supabaseClient
    .from('anak_asuhan')
    .select('*', { count: 'exact', head: true });
  const countPegawai = await supabaseClient
    .from('pegawai')
    .select('*', { count: 'exact', head: true });
  const countBarang = await supabaseClient
    .from('barang_anak_asuhan')
    .select('*', { count: 'exact', head: true });

  if (countAnakAsuhan.error) {
    throw new Boom.internal(countAnakAsuhan.error);
  }

  if (countPegawai.error) {
    throw new Boom.internal(countPegawai.error);
  }

  if (countBarang.error) {
    throw new Boom.internal(countBarang.error);
  }
